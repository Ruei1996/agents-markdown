...
Use `list_directory` and `code_search` to locate all documentation files:
...
1. `read_file` the doc and list every function, API endpoint, class, CLI flag, or behavior it describes
2. Use `code_search` to locate the corresponding source code
3. `read_file` the source to check for discrepancies:
